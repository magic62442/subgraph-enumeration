...
size_t gNumInterSection = 0;
size_t gIntersectionSize = 0;
...
ui ComputeSetIntersection::BinarySearch(const std::vector<VertexID> &array, VertexID target) {
    ui offset_begin = 0;
    ui offset_end = array.size();
    while (offset_end - offset_begin >= 16) {
        auto mid = (offset_begin + offset_end) / 2;
#ifndef __APPLE__
...
bool ComputeSetIntersection::Contain(const VertexID *src, const ui begin, const ui end, const ui target) {
    ui offset_begin = begin;
    ui offset_end = end;
    while (offset_end - offset_begin >= 16) {
        auto mid = (offset_begin + offset_end) / 2;
...
void ComputeSetIntersection::LeapfrogJoin(const VertexID **arrays, ui *counts, ui num, VertexID *cn, ui &cn_count) {
    if (num == 1) {
        memcpy(cn, arrays[0], counts[0] * sizeof(VertexID));
        cn_count = counts[0];
        return;
    }
...
void ComputeSetIntersection::MultiIntersection(const VertexID **arrays, ui *counts, ui num,
                                               VertexID *cn, ui &cn_count) {
    if (num == 1) {
        memcpy(cn, arrays[0], counts[0] * sizeof(VertexID));
        cn_count = counts[0];
...
#endif
//        for (int i = 0; i < num; ++i) {
//            gIntersectionSize += counts[i];
//        }

    // Step 1: 使用冒泡排序按数组长度升序排列 arrays 和 counts
...
//    }
    ...
    // Step 2:
    ComputeCandidates(arrays[0], counts[0], arrays[1], counts[1], cn, cn_count);
    ...
    for (ui i = 2; i < num; ++i) {
        ...
        ComputeCandidates(cn, cn_count, arrays[i], counts[i], cn, cn_count);
    }
}
