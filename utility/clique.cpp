...
void kCliqueCount(int l, int K, specialsparse *g, unsigned *vertices, HashTable h, int orbitType) {
    unsigned i, j, k, end, u, v, w;
    ...
    if (l == K)
    {
        ...
        for (i = 0; i < g->ns[l]; i++) {
            ...
            kCliqueCount(l - 1, K, subg, vertices, h, orbitType);
            for (j = 0; j < g->ns[l - 1]; j++) {//restoring labels
                ind[loc[j]] = -1;
                v = g->sub[l - 1][j];
                g->lab[v] = l;
    ...
    else
    {
        ...
        for (int i = 0; i < g->ns[l]; i++) {
            ...
            kCliqueCount(l - 1, K, g, vertices, h, orbitType);
